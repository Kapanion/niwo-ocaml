...
let rec pretty_print ?indent t =
  let indent = Option.value indent ~default:0 in
  ...
  | Fun (name, None, params) ->
    ...
    ^ name
    ^ "("
    ^ String.concat ~sep:"," (List.map ~f:var_to_string params)
    ^ ")"
  | Fun (name, Some idx, params) ->
    indent_str
    ^ name
  ...
  | Exists (vars, f) ->
    indent_str
    ^ "∃ "
    ^ String.concat ~sep:"," (List.map ~f:(var_to_string ~annotate:true) vars)
    ^ ".\n"
  ...
  | ForallOtherThan (vars, otherthan, f) ->
    ...
    ^ String.concat ~sep:"," (List.map ~f:(var_to_string ~annotate:true) vars)
    ^ " ∉ {"
    ^ String.concat ~sep:"," (List.map ~f:(var_to_string ~annotate:true) otherthan)
    ^ "}.\n"
    ^ pretty_print ~indent f
;;
...
(* Encode Fun to Var *)
let encode_fun_to_var t =
  match t with
  | Fun (name, None, params) ->
    let param_str =
      params |> List.map ~f:(fun v -> "_" ^ v.name ^ "#" ^ v.typ) |> String.concat ~sep:""
    in
